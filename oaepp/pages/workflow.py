    ...
    def _rule_config_section():
        return rx.box(
            rx.vstack(
                ...
                rx.hstack(
                    rx.checkbox(
                        checked=WorkflowState.require_pr_on_close,
                        on_change=WorkflowState.set_require_pr_on_close,
                    ),
                ...
                rx.hstack(
                    rx.checkbox(
                        checked=WorkflowState.require_merged_pr,
                        on_change=WorkflowState.set_require_merged_pr,
                    ),
                ...
                rx.hstack(
                    rx.input(
                        ...
                        on_change=WorkflowState.set_course_id_input,
                    ),
                    rx.button("添加课程规则", on_click=WorkflowState.add_course_rule_ui, size="sm"),
                    spacing="3",
                ),
                rx.cond(
                    ...
                    rx.vstack(
                        rx.foreach(
                            WorkflowState.course_rules,
                            lambda item, i: _course_rule_card(item),
                        ),
                        spacing="3",
                        width="100%",
                    ),
            ...
            rx.box(
                rx.box(
                    rx.vstack(
                        ...
                        rx.text("请输入与此 Issue 关联的 PR 编号并实时验证", color="gray", size="2"),
                        rx.input(
                        ...
                        rx.cond(
                            ...
                            rx.box(
                                rx.cond(
                                    ...
                                    rx.vstack(
                                        rx.hstack(rx.text("✓", color="green"), rx.text(WorkflowState.pr_validation_result.message, color="green"), spacing="2"),
                                        rx.text(f"标题: {WorkflowState.pr_validation_result.title}", size="1"),
                                        rx.text(f"状态: {WorkflowState.pr_validation_result.state}", size="1"),
                                        rx.text(f"合并: {'已合并' if WorkflowState.pr_validation_result.is_merged else '未合并'}", size="1"),
                                        spacing="1",
                                    ),
                                    rx.hstack(rx.text("✗", color="red"), rx.text(WorkflowState.pr_validation_result.message, color="red"), spacing="2"),
                                ),
                                padding="12px",
                                border_radius="8px",
                                background=rx.cond(WorkflowState.pr_validation_result.valid, "#f0fdf4", "#fef2f2"),
                                width="100%",
                            ),
                        ),
                        rx.hstack(
                            rx.button("取消", on_click=WorkflowState.close_pr_modal, variant="outline"),
                            rx.button("确认关闭 Issue", on_click=WorkflowState.confirm_close_issue, color_scheme="red"),
                            justify="end",
                            spacing="3",
                            width="100%",
                        ),
        ...
        return rx.box(
            rx.vstack(
                ...
                rx.cond(
                    ...
                    rx.box(rx.text("暂无警告记录，Webhook 正在监听中", color="gray"), padding="24px", text_align="center"),
                ),
                spacing="4",
                width="100%",
            ),
        ...
        return rx.box(
            rx.vstack(
                ...
                rx.cond(
                            ...
                            lambda a, i: rx.box(
                                rx.vstack(
                                    rx.hstack(
                                        ...
                                        rx.box(rx.text(f"PR #{a.pr_number}", size="1"), padding="2px 8px", border_radius="4px", background="#dbeafe"),
                                        rx.cond(
                                            a.pr_is_merged,
                    ...
                    rx.box(rx.text("暂无关联记录", color="gray"), padding="24px", text_align="center"),
                ),
                spacing="4",
                width="100%",
            ),
            ...
            rx.vstack(
                ...
                rx.box(
                    rx.vstack(
                        ...
                        rx.text("配置路径: GitHub 仓库 → Settings → Webhooks → Add webhook", size="2", color="gray"),
                        spacing="2",
                    ),
                    padding="12px",
                    background="#f8fafc",
    ...
    def _demo_issue_section():
        return rx.box(
            rx.vstack(
                ...
                rx.text("在下方输入 PR 编号来模拟关闭流程（验证弹框及实时校验）", color="gray", size="2"),
                rx.hstack(
                    rx.input(
                        placeholder="输入 PR 编号",
                        value=WorkflowState.pr_number_input,
                    ...
                    rx.button("开始关闭", on_click=WorkflowState.open_pr_modal, color_scheme="red"),
                    spacing="3",
                ),
                spacing="4",
                width="100%",
