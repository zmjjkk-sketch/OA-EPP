...
- course_rules: 各课程的独立规则配置（List，兼容 rx.foreach）
...
if rx is not None:
    class WorkflowState(rx.State):
        ...
        course_rules: List[Dict[str, Any]] = []
        ...
        def _find_course_rule(self, course_id: str) -> Optional[Dict[str, Any]]:
            for r in self.course_rules:
                if r.get("course_id") == course_id:
                    return r
            return None

        def _course_rule_index(self, course_id: str) -> int:
            for i, r in enumerate(self.course_rules):
                if r.get("course_id") == course_id:
                    return i
            return -1

        def get_course_rule(self, course_id: str) -> Dict[str, Any]:
            found = self._find_course_rule(course_id)
            if found:
                return found
            return {
                "course_id": course_id,
        ...
        def set_course_rule(self, course_id: str, require_pr_on_close: bool, require_merged_pr: bool):
            idx = self._course_rule_index(course_id)
            entry = {
                "course_id": course_id,
            ...
            if idx >= 0:
                self.course_rules[idx] = entry
            else:
                self.course_rules.append(entry)

        def remove_course_rule(self, course_id: str):
            self.course_rules = [r for r in self.course_rules if r.get("course_id") != course_id]

        def add_course_rule_ui(self):
            cid = self.course_id_input.strip()
            if cid:
                self.set_course_rule(cid, self.require_pr_on_close, self.require_merged_pr)
                self.course_id_input = ""
                self.toast_message = f"课程 {cid} 规则已添加（继承全局设置）"
        ...
        async def _validate_pr_via_api(self, owner: str, repo: str, pr_number: int) -> Dict[str, Any]:
            try:
                ...
                if response.status_code == 404:
                    return {"valid": False, "exists": False, "message": f"PR #{pr_number} 不存在"}
                if response.status_code != 200:
                    return {"valid": False, "exists": False, "message": f"GitHub API 请求失败: {response.status_code}"}
                ...
                return {"valid": False, "exists": False, "message": f"验证失败: {str(e)}"}
        ...
        def open_pr_modal(self):
            issue = {"number": self.pr_number_input, "title": "示例 Issue（请替换为实际 Issue）"}
        ...
        async def _do_close_issue(self):
            ...
            rule = self.get_course_rule(self.course_id_input or "default")
            ...
            if rule.get("require_merged_pr", False) and not validation.get("is_merged"):
                self.pending_close_data = {
                    ...
                    "pr_number": pr_num,
                    "pr_info": validation,
                    "course_id": self.course_id_input or "default",
                }
                self.show_unmerged_confirm = True
                return

            await self._record_association(issue.get("number", 0), pr_num, validation, self.course_id_input or "default", closed_by)
            self.show_pr_modal = False
            self.current_issue = None
            self.pr_number_input = ""
            self.pr_validation_result = {}
        ...
        async def _do_unmerged_close(self, data: Dict[str, Any]):
            ...
            await self._record_association(issue.get("number", 0), pr_num, pr_info, course_id, "teacher")
            self.show_pr_modal = False
            self.current_issue = None
            self.pr_number_input = ""
            self.pr_validation_result = {}
